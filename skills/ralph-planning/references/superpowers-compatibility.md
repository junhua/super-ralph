...
| **superpowers:subagent-driven-development** | As a monolithic skill, incompatible: assumes a persistent orchestrator with cross-task conversation memory. However, its individual quality gates (self-review, adversarial spec review, code quality review) are fully self-contained within a single subagent dispatch and work within a single ralph-loop iteration. | Use hybrid mode execution prompt, which incorporates the compatible quality gates from subagent-driven-development: enriched implementer prompts with self-review, adversarial spec compliance review ("Do Not Trust the Report"), and code quality review. Cross-iteration state is recovered from git log and file inspection. |
